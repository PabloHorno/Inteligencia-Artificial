...
public:
	...
	void set_texto_temperatura(const std::string&& texto)
	{
		panel.temperatura->setText(texto);
	}
    void actualiza_recorrido(const std::vector<unsigned>& ind,const std::string&& texto1, const std::string&& texto2, const std::string&& texto3)
    {
        std::lock_guard<std::mutex>lock(barrera_render);    //Bloqueamos render
        panel.iteracion->setText(texto1);
		panel.mejor_distancia->setText(texto2);
		panel.temperatura->setText(texto3);

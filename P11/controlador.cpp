	...
	bool Controlador::carga_problema_tsplib()
	{
		...
		if (lee_nombre_fichero(fichero_tsp))
		{
			...
			if (set_coordenadas_visualizacion(ciudades))
			{
				...
				vista.set_texto_distancia_optima("Óptimo : " + parche::to_string(ciudades.calcula_distancia(ciudades.get_codigo_optimo())));
		...
		while (!fin_algoritmo)
		{
			...
			if (fin_algoritmo)
			{
				while (!datos.empty())
				{
					...
					datos.pop();
					vista.actualiza_recorrido(dato.recorrido, "Iteración: " + parche::to_string(dato.iteracion), "Distancia: " + parche::to_string(dato.mejor_distancia));
				}
			}
			else
			{

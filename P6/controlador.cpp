...
namespace controlador
{
	void Controlador::registra_observadores()
	{
		...
		observador_vista.registra_observador(Evento_Vista::ejecutar, std::bind(&Controlador::ejecuta_sa_tsp, this));
		...
		observador_modelo.registra_observador(Evento_Modelo::nuevo_recorrido, std::bind(&Controlador::push_datos, this));
	}
	...
	bool Controlador::carga_problema_tsplib()
	{
		...
		if (lee_nombre_fichero(fichero_tsp))
		{
			...
			if (set_coordenadas_visualizacion(ciudades))
			{
				...
				vista.set_texto_distancia_optima("Óptimo : " + parche::to_string(ciudades.calcula_distancia(ciudades.get_recorrido_optimo())));
				vista.set_texto_iteracion("");
				vista.set_texto_mejor_distancia("");
				ruta.inicializa_ruta(std::move(ciudades));
				//Eliminar la llamada de inicialización
	...
	{
		inicio_vista = true;
		//La vista nos garantiza que todos sus recursos están listos
	}
	...
	void Controlador::ejecutar()
	{
		...
		//o elegidas por línea de comandos
	...
	void Controlador::push_datos()
	{
		...
		datos.push({ algoritmo_sa_tsp.get_recorrido(),algoritmo_sa_tsp.get_mejor_distancia(),algoritmo_sa_tsp.get_iteracion_actual(), algoritmo_sa_tsp.get_temperatura() });
		...
		while (!fin_algoritmo)
		{
			...
			if (fin_algoritmo)
			{
				while (!datos.empty())
				{
					...
					datos.pop();
					vista.actualiza_recorrido(dato.recorrido, "Iteración: " + parche::to_string(dato.iteracion), "Distancia: " + parche::to_string(dato.mejor_distancia), "Temperatura: " + parche::to_string(dato.temperatura));
				}
			}
			else
			{

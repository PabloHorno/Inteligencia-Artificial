...
namespace nodo_reinas
{
	...
	bool Nodo_reinas::sucesor_aleatorio(std::mt19937& rng)
	{
		...
		if (!columnas.empty())
		{
			...
			posicion.push_back(columnas[aleatorio(rng)]);
			return true;
		}
		return false;
	}
	...
	{
		if (columna.find(col_hijo) == columna.end()) //La columna no está ocupada
			if (diag_45.find(col_hijo + fil_hijo) == diag_45.end()) //Diagonal 45 no está ocupada
				if (diag_135.find(col_hijo - fil_hijo) == diag_135.end()) //Diagonal 135 no está ocupada
					return true;
		return false;
	}
	vector<Nodo_reinas_set> Nodo_reinas_set::expandir() const
	{
		...
		for (unsigned col_hijo = 0; col_hijo != dim; ++col_hijo) //Todas las posibles columnas
		{
			if (columna_valida(fil_hijo, col_hijo))
			{
				auto hijo = *this;
				hijo.posicion.push_back(col_hijo);
				hijo.columna.insert(col_hijo);
				hijo.diag_135.insert(col_hijo - fil_hijo);
				hijo.diag_45.insert(col_hijo + fil_hijo);
				lista_hijos.push_back(hijo);
			}
		}
		return lista_hijos;
	...
	bool Nodo_reinas_set::sucesor_aleatorio(std::mt19937& rng)
	{
		...
		{
			std::uniform_int_distribution<unsigned>aleatorio(0, columnas.size() - 1);
			unsigned col_hijo = columnas[aleatorio(rng)];
			posicion.push_back(col_hijo);
			this->columna.insert(col_hijo);
			this->diag_135.insert(col_hijo - fil_hijo);
			this->diag_45.insert(col_hijo + fil_hijo);
			return true;
		}
		return false;
	}

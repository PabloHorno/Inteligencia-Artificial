...
var x{i in Agentes, j in Tareas}, binary;

/* El objetivo es lograr la asignación más económca */
minimize obj: sum{i in Agentes, j in Tareas} coste[i,j] * x[i,j];

/* Cada agente debe realizar una única tarea */
s.t. restriccion_1{i in Agentes}: sum{j in Tareas} x[i,j] = 1;

/* Cada tarea debe asignarse a un único agente */
s.t. restriccion_2{j in Tareas}: sum{i in Agentes} x[i,j] = 1;

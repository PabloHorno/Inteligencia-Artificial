...
int Posicion_reversi::evalua_posicion(const bool es_terminal) const
{
   //La heuristica da valores entre [-get_filas()*get_columnas(),get_filas()*get_columnas()]
    ...
    if(es_terminal) //La puntuación de una posición terminal ganadora siempre será mayor que la
    {               //puntuación de una heurística. Es mejor saber que ganas seguro por 3 fichas
                    //que una heurística evalúe que posiblemente ganes por 10 fichas
       if(valor>0)
       {
